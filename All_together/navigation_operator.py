...
from utils import parse_object, get_cart_or_basket_id


# This is the agent for navigating to critical objects
# The agent uses planning using a BFS algorithm to find the best path
...
class navigation_operator:
    def __init__(self, sock_game:socket.socket,target_object, config_file_path:str,
                 player_num = 0, player_id = 0) -> None:
    
        with open(config_file_path) as f:
            yaml_data = yaml.safe_load(f)

        self.sock_game = sock_game 
        parsed_obj = parse_object(target_object)

        self.player_num = player_num
        self.player_id = player_id


        #Planning agent that takes care of all navigation
        self.navi_agent = PlanningAgent(
            name=parsed_obj, 
            target=parsed_obj, 
            playerNum=self.player_id
        ) 
        self.navi_agent.build_map()

        # get target pose
        self.target_object = target_object    
        if self.target_object in yaml_data["TARGET_CART_POSITION"].keys():
            self.target_pose = yaml_data["TARGET_CART_POSITION"][self.target_object]
        elif self.target_object in yaml_data["SIMPLE_MOVE_OBJECTS"].keys():
            self.target_pose = yaml_data["SIMPLE_MOVE_OBJECTS"][self.target_object]
        elif self.target_object in yaml_data["WITH_SPECIAL_DIRECTION"].keys():
            self.target_pose = yaml_data["WITH_SPECIAL_DIRECTION"][self.target_object]

        #Load variables for discretization and state space
    ...
    #Checker to see if agent within target distance
    def reach_target_checker(self, state):
        agent_position = state['observation']['players'][self.player_id]['position']
    ...
    def do_navigation(self):
        ...
        self.sock_game.send(str.encode(f"{self.player_id} NOP"))
        ...
        while not state['gameOver']:
            ...
            # get action
            action_index, _ = self.navi_agent.choose_action(state)
            action = f"{self.player_id} " + self.action_commands[action_index]
...
class simple_movement_operator:
    ...
    def do_simple_move(self):
        ...
        state = recv_socket_data(self.sock_game)
        state = json.loads(state)
        current_agent_pos = state['observation']['players'][self.player_id]['position']
        action = f"{self.player_id} " + self.simple_move_direction
...
# code for checkout, the code may looks werid due to some faulty thinking and bad code design :-(
    ...
    def checkout_good(self):
        ...
        action_north = f"{self.player_id} NORTH"
        action_south = f"{self.player_id} SOUTH"
        action_east = f"{self.player_id} EAST"
        action_west = f"{self.player_id} WEST"
        action_int = f"{self.player_id} INTERACT"
        ...
        if self.cart_or_basket == "carts":
            ...
            my_cart_or_basket_id = get_cart_or_basket_id(state, self.player_id, self.cart_or_basket)
            curr_cart_pose = state['observation']['carts'][my_cart_or_basket_id]['position']
            ...
            state = recv_socket_data(self.sock_game)
            state = json.loads(state)
            agent_pose_before_checkout = state['observation']['players'][self.player_id]['position']
            # drop the cart
            self.pick_and_drop_cart()
        ...
        # in case of basket we need to move west a few step to prevent from being stuck
            ...
            state = recv_socket_data(self.sock_game)
            state = json.loads(state)
            curr_agent_pose = state['observation']['players'][self.player_id]['position']
    ...
    def head_initial_direction(self):
        action = f"{self.player_id} " + self.initial_direction
        print("initial action", action)
        self.sock_game.send(str.encode(action))
        next_state = recv_socket_data(self.sock_game)
        next_state = json.loads(next_state)
        return next_state['observation']['players'][self.player_id]['position']

    # pick up or drop the cart
    def pick_and_drop_cart(self):

        action = f"{self.player_id} TOGGLE_CART"
        self.sock_game.send(str.encode(action))
        state = recv_socket_data(self.sock_game)
        state = json.loads(state)

        if state['observation']['players'][self.player_id]['curr_cart'] == -1:
            print("drop cart!")
        elif state['observation']['players'][self.player_id]['curr_cart'] != -1:
            print("pick cart!")
    ...
    def go_to_shelf(self, direction, shelf_interact_pose, axis_we_care):
        ...
        action = f"{self.player_id} " + direction
        self.sock_game.send(str.encode(f"{self.player_id} NOP"))
        state = recv_socket_data(self.sock_game)
        state = json.loads(state)
        current_agent_pos = state['observation']['players'][self.player_id]['position']
    ...
    def pickup_goods(self):
        ...
        pick_time = 0
        cart_or_basket_id = get_cart_or_basket_id(state, self.player_id, self.cart_or_basket)
        initial_quants = state['observation'][self.cart_or_basket][cart_or_basket_id]['contents_quant']
        while pick_time < self.max_try_time:
            if state['observation']['players'][self.player_id]['holding_food'] is not None or state['observation'][self.cart_or_basket][0]['contents_quant'] != initial_quants:
                print("got {}".format(state['observation']['players'][self.player_id]['holding_food']))
                break
            else:
                self.sock_game.send(str.encode(f"{self.player_id} INTERACT"))
                state = recv_socket_data(self.sock_game)
                state = json.loads(state)
                # print(state['observation'][self.cart_or_basket][0]['contents_quant'])
            pick_time += 1
        if state['observation']['players'][self.player_id]['holding_food'] is None and state['observation'][self.cart_or_basket][0]['contents_quant'] == initial_quants:
    ...
    def put_good_to_cart(self, axis_we_care):
        ...
        while put_back_time < self.max_try_time:
            if quants != initial_quants:
                ...
                state = recv_socket_data(self.sock_game)
                state = json.loads(state)
                quants = state['observation'][self.cart_or_basket][cart_or_basket_id]['contents_quant']
                print(quants)
            put_back_time += 1
...
class general_shopping:
    def __init__(self, sock_game:socket.socket, target_object_list, cart_or_basket,config_file_path,
                 player_num = 0, player_id = 0) -> None:
        self.sock_game = sock_game
        ...
        for target_object in self.target_object_list:
            ...
            if target_object in self.navigation_target_list:
                # move_operator = PlanningAgent(name="", target=parse_object(target_object),playerNum=1)
                # move_operator.build_map()
                move_operator = navigation_operator(self.sock_game, target_object, config_file_path,
                                                    player_num=self.player_num, player_id=self.player_id)
            else:
                move_operator = navigation_operator(self.sock_game, target_object, config_file_path,
                                                    player_num=self.player_num, player_id=self.player_id)
                # move_operator = PlanningAgent(name="", target=parse_object(target_object),playerNum=1)
...
class start_shopping_operator:
    ...
    def __init__(self, sock_game:socket.socket,cart_or_basket, config_file_path, 
                 player_num = 0, player_id = 0) -> None:
        self.sock_game = sock_game
        self.cart_or_basket = cart_or_basket
        self.player_num = player_num
    ...
    def get_cart_or_basket(self):
        ...
        while abs(current_agent_pos[1] - self.desired_y_axis) >= self.interaction_dist:
            self.sock_game.send(str.encode(f"{self.player_id} SOUTH"))
            state = recv_socket_data(self.sock_game)
            state = json.loads(state)
            current_agent_pos = state['observation']['players'][self.player_id]['position']
        ...
        state = recv_socket_data(self.sock_game)
        state = json.loads(state)
        cart_or_basket_num_base = len(state['observation'][self.cart_or_basket])
        cart_or_basket_num_new = len(state['observation'][self.cart_or_basket])
        cart_or_basket_num = cart_or_basket_num_new - cart_or_basket_num_base
    ...
    def return_cart_or_basket(self):
        ...
        if self.cart_or_basket == "carts":
            curr_pose = state['observation']['carts'][self.player_id]['position']
        elif self.cart_or_basket == "baskets":
            curr_pose = state['observation']['players'][self.player_id]['position']
...
class leave_operator:
    ...
    def leave(self):
        ...
        while cnt < self.movement_limit:
            self.sock_game.send(str.encode(f"{self.player_id} WEST"))
            state = recv_socket_data(self.sock_game)
            state = json.loads(state)
            cnt += 1
...
if __name__ == "__main__":
    ...
    # sock_game.send(str.encode(f"{self.player_id} HACK"))
