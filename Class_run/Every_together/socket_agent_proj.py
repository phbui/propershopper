...
class Agent:
    ...
    def wait(self):
        """Wait in the corner of solitude for the world to become a better place"""
        self.goto(goal=CORNER_OF_SOLITUDE, is_item=False)
        if self.goal_status == 'failure':
            self.goal = ''
            return
        # wait for 100 time step
        for _ in range(100):
    ...
    def goto(self, goal, is_item=True):
        ...
        if is_item:  # goal is an item in the env, not a (x, y)
            ...
            if goal in ('cartReturn 0', 'cartReturn 1'):  # access these from the North
                ...
                if self.container_type != goal:  # current container and goal doesn't match, get current container instead
                    ...
                    if goal == 'cart':
                        interact_box = list(interact_boxes.values())[
                            0]  # cart only has one interact box, go to that interact box
                        goal = self.interact_box_to_goal_location(box=interact_box)
                    else:
        ...
        print(f"Agent {self.agent_id} going to location {self.goal} at {goal} with container {self.container_type}")

        for box_region in path:
            if box_region.name == "W_walkway" or box_region.name == "E_walkway":
                self.reactive_nav(goal=box_region, align_x_first=True,
                                  is_box=True)  # to navigate to a walkway, align x first
            else:
                self.reactive_nav(goal=box_region, align_x_first=False,
                                  is_box=True)  # to navigate to any other region, align y first
    ...
    def get_item(self):
        ...
        if not self.holding_container:  # must be holding a container
            ...
            return
        if len(self.env['observation'][self.container_type + 's'][self.container_id]['contents']) == \
                self.env['observation'][self.container_type + 's'][self.container_id]['capacity']:
            self.goal_status = 'failure'
    ...
    def get_container(self):
        ...
        self.holding_container = self.container_type == 'basket' and self.container_id != -1  # we have to make this assumption, it's not reflected in the env
        # else:
    ...
    def strategically_choose_from_shopping_list(self, shopping_list):
        ...
        for item in shopping_list:
            crowdedness = calculate_crowdedness_factor(self.agent_id, locs[item]['interact_boxes']['SOUTH_BOX'],
    ...
    def interact_box_to_goal_location(self, box: dict):
        ...
        while True:
            ...
            if self.holding_container and self.container_type == 'cart':  # change player's shape to account for cart
                ...
                if player_cart['direction'] == Direction.NORTH.name:
                    player_cart['position'][1] -= cart[
                        'height']  # need to move the y of the player to the upper left of cart + player
                    ...
                    player_cart['position'][0] -= cart[
                        'width']  # need to move the x of the player to the upper left of cart + player
            ...
            if is_box:
                if isinstance(goal, BoxRegion):  # it's a box region
                    ...
                    if not (self.holding_container and self.container_type == 'cart') and can_interact_in_box(
                            player=player, interact_box=goal):
                        break
                    elif (self.holding_container and self.container_type == 'cart') and loc_in_box(
                            loc=player['position'],
                            box=goal):  # we are holding a cart, it's good enough to be in the interaction box. We don't have to face the right direction
                        break
            ...
            if reached_x and reached_y:
                if not is_box:
                    ...
                    if not isinstance(goal, BoxRegion) and not (
                            self.holding_container and self.container_type == 'cart'):  # it's an interact box, we have to face the right direction
            ...
            if self.holding_container and self.container_type == 'basket':
                player[
                    'curr_basket'] = self.container_id  # a hack. curr_basket isn't reflected in the env unlike curr_cart
            ...
            while project_collision(player, self.env, command, dist=STEP):
                ...
                if stuck >= 20:  # been stuck for too long, it's probably a static corner some other agent created, F it, try a random action
                    self.execute('NOP') # to observe the updated env
                    command = random.choice(
                        [dir for dir in Direction if (dir != Direction.NONE and dir != original_command)])
    ...
    def _ninety_degrees(self, dir: Direction):
...
if __name__ == "__main__":
    ...
    agents = [Agent(sock_game, 4, env=output)]
