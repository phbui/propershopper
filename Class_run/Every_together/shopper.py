...
if __name__ == "__main__":
    ...
    #Get player ID for agent to control
    sock_game.send(str.encode(f"{player_id} NOP"))
    state = recv_socket_data(sock_game)
    state = json.loads(state)

    raw_shopping_list = state['observation']['players'][player_id]['shopping_list']
    list_quant = state['observation']['players'][player_id]['list_quant']
